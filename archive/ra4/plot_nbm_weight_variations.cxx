...
int main(){
  ...
  auto data_1l = Process::MakeShared<Baby_full>("Data", Process::Type::data, kBlack,
    {"/net/cms2/cms2r0/babymaker/babies/2016_11_08/data/merged_database_standard/*.root"},"pass&&trig_ra4");
  ...
  for(int ilep=0; ilep<3; ilep++){
    ...
    for(int inveto=0; inveto<2; inveto++){
      ...
      for(int injets=0; injets<4; injets++){
        NamedFunc njetscut(true);
        if(injets==0)  njetscut="njets>=5";
        if(injets==1)  njetscut="njets>=6";
	if(injets==2)  njetscut="njets>=2";
	if(injets==3)  njetscut="njets>=3";
...
//                          full_trig_skim_1l, all_plot_types);
	  ...
	  pm.Push<Hist1D>(Axis(7, -0.5, 6.5, "nbm", "N_{b}", {0.5, 1.5, 2.5}),
                          lepcut&&lowmtcut&&njetscut&&nvetocut,
                          full_trig_skim_1l, all_plot_types).Weight("(weight/(w_isr*(w_isr>0)+1*(w_isr<=0)))");
	  pm.Push<Hist1D>(Axis(7, -0.5, 6.5, "nbm", "N_{b}", {0.5, 1.5, 2.5}),
                          lepcut&&lowmtcut&&njetscut&&nvetocut,
                          full_trig_skim_1l, all_plot_types).Weight("(weight/w_btag)");
	  pm.Push<Hist1D>(Axis(7, -0.5, 6.5, "nbm", "N_{b}", {0.5, 1.5, 2.5}),
                          lepcut&&lowmtcut&&njetscut&&nvetocut,
                          full_trig_skim_1l, all_plot_types).Weight("(weight/((w_isr*(w_isr>0)+1*(w_isr<=0))*w_btag))");
