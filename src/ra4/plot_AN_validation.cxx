...
int main() {
  ...
  string data18_path("/net/cms2/cms2r0/babymaker/babies/2019_03_30/data/merged_database_standard/");
	...
	auto data_2016 = Process::MakeShared<Baby_full>("2016 Data",data,kBlack,  
	                 {data16_path+"*.root"},"trig_ra4&&"+q_cuts && Functions::trig_run2);
	auto data_2017 = Process::MakeShared<Baby_full>("2017 Data",data,kBlack,
	                 {data17_path+"*.root"},"trig_ra4&&"+q_cuts && Functions::trig_run2);
	auto data_2018 = Process::MakeShared<Baby_full>("2018 Data",data,kBlack,
	                 {data18_path+"*.root"},"trig_ra4&&"+q_cuts && Functions::hem_veto && Functions::trig_run2);
  ...
  auto mcRunII_other    = Process::MakeShared<Baby_full>("Other", back, colors("other"),
	...
	vector<vector<shared_ptr<Process> >> data_mc = {data16_mc16, data17_mc17, data18_mc17, data_mc_RunII};
  ...
  PlotOpt lin_stack_info = log_lumi().YAxis(YAxisType::linear); 
	...
	vector<double> sample_lumi = {35.9, 41.5, 60, 137};
	string tag;
  NamedFunc w_tot(Functions::wgt_run2 * Functions::eff_trig_run2);
	const vector<double> met_bins = {100,150,200,350,500,800};
    ...
    pm.Push<Hist1D>(Axis(met_bins, "met",  "p_{T}^{miss} [GeV]",{}), 
		                BaselineCuts("met"),   temp, log_stack).Weight(w_tot).Tag(tag+"_AltBinning");
    ...
    pm.Push<Hist1D>(Axis(met_bins,  "met", "p_{T}^{miss} [GeV]",{}), 
	                  BaselineCuts("met","2l"),  temp, log_stack).Weight(w_tot).Tag(tag+"_AltBinning");
    pm.Push<Hist1D>(Axis(15,500,  2000, "st",  "S_{T} [GeV]",{}), 
    ...
    pm.Push<Hist1D>(Axis(met_bins, "met",  "p_{T}^{miss} [GeV]",{}), 
	                  BaselineCuts("met","56j"),  temp, log_stack).Weight(w_tot).Tag(tag+"_AltBinning");
    pm.Push<Hist1D>(Axis(15,500,  2000, "st",  "S_{T} [GeV]",{}), 
