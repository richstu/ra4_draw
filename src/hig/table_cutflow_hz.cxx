...
int main(){
  ...
  string folderHH(bfolder+"/cms2r0/babymaker/babies/2017_03_17/TChiHH/merged_higmc_unskimmed/");
  string folderHZ(bfolder+"/cms2r0/babymaker/babies/2017_06_01/TChiHZ/merged_higmc_unskimmed/");
  ...
  for (unsigned isig(0); isig<sigm.size(); isig++) {
    procs.push_back(Process::MakeShared<Baby_full>("TChiHH("+sigm[isig]+",1)", 
      Process::Type::signal, 1, {folderHH+"*TChiHH_mGluino-"+sigm[isig]+"*.root"}, "1"));
    procs.push_back(Process::MakeShared<Baby_full>("TChiHZ("+sigm[isig]+",1)", 
      Process::Type::signal, 1, {folderHZ+"*TChiHZ_*"+sigm[isig]+"*.root"}, "1"));
  }
  ...
  NamedFunc br_wgt("br_wgt", [&](const Baby &b){
    float wgt_ = 1;
    if (b.type()==-999999){
      int nh(0), nh_nonbb(0);
      ...
      nh_nonbb /=2;
      wgt_ *= (br*br/.25*(nh==2) + 2*br*(1-br)/.5*(nh==1) + (1-br)*(1-br)/.25*(nh==0));
      // wgt_ *= (nh_nonbb==0);
    }
    return wgt_;
  });
  ...
  string sigonly = "(type>100e3 || type==-999999)";

  string ncols = to_string(procs.size()+1);  
  PlotMaker pm;
  pm.Push<Table>("cutflow", vector<TableRow>{
  TableRow("No selection                ", 
    sigonly,0,0, Higfuncs::weight_higd * br_wgt * "1/eff_jetid"),
  TableRow("$0\\ell$, $\\text{4-5 jets}$  ", 
    baseline+"&&"+sigonly,0,0, Higfuncs::weight_higd * br_wgt * "1/eff_jetid"),
  TableRow("$N_{\\text{b,T}}\\geq 2$      ", 
    baseline+"&&"+sigonly + " &&" +c_2bt,0,0, Higfuncs::weight_higd * br_wgt * "1/eff_jetid"),
